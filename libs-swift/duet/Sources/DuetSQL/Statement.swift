...
public extension SQL.Statement {
  ...
  internal static func create<M: Model>(
    ...
    case .updateAllExcept(let except):
      // always exclude lifecycle columns from a conflict update: `deleted_at` would
      // otherwise resurrect a soft-deleted row, and `id`/`created_at` must stay pinned
      // to the surviving row
      let excluded = Set(except.map { M.columnName($0) })
        .union(["id", "created_at", "deleted_at"])
