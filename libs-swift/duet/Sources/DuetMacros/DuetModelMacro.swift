...
extension DuetModelMacro: ExtensionMacro {
  public static func expansion(
    ...
    let access = accessPrefix(of: structDecl)
    let conformance = protocols.isEmpty ? "" : ": DuetSQL.Model"
...
private func parseArguments(from node: AttributeSyntax) -> MacroArguments? {
  ...
  return MacroArguments(schema: schema ?? "public", table: table, clientCreatedAt: clientCreatedAt)
}
...
enum DuetModelDiagnostic: String, DiagnosticMessage {
  ...
  var message: String {
    ...
    case .invalidArguments:
      "@DuetModel requires a `table:` string literal argument"
    }
  }
