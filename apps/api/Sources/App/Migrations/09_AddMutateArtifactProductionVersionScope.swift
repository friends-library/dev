...
struct AddMutateArtifactProductionVersionScope: AsyncMigration {
  ...
  func prepare(on database: Database) async throws {
    get(dependency: \.logger).info("Running migration: AddMutateArtifactProductionVersionScope UP")
  ...
  func revert(on database: Database) async throws {
    get(dependency: \.logger)
      .info("Running migration: AddMutateArtifactProductionVersionScope DOWN")
    _ = try await database.enum(TokenScope.M5.dbEnumName)
