...
final class RateLimitedSlackClientTests: AppTestCase, @unchecked Sendable {
  func testRateLimitedClient() async throws {
    ...
    await withDependencies {
      $0.logger = Logger(label: "mock") { _, _ in handler }
    } operation: {
      ...
      await withDependencies {
        ...
        await client.send(.init(
          text: "Unusual missing location data:\n```foo\n```",
          channel: .info,
        ))
      }
      await expect(sent.value).toEqual([])
      ...
      let dayString = formatter.string(from: .epoch)

      // but then the day changes, so we reset the limit
      await withDependencies {
        $0.date = .constant(Date(timeIntervalSince1970: 86401))
      } operation: {
