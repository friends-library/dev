...
import type { DocumentWithMeta, NewsFeedType } from '@/lib/types';
...
export async function getNewsFeedItems(
  ...
  documents: {
    en: Array<
      Pick<
        DocumentWithMeta,
        'authorName' | 'dateAdded' | 'id' | 'title' | 'authorSlug' | 'slug'
      >
    >;
    es: Array<
      Pick<
  ...
  const recentAdditions = getRecentAdditions(`en`, documents.en)
    .concat(getRecentAdditions(`es`, documents.es))
...
function getRecentAdditions(
  ...
  return documents
    ...
    .map((doc) => ({
      month: months[LANG][new Date(doc.dateAdded).getMonth()]?.substring(0, 3) || ``,
      day: String(new Date(doc.dateAdded).getDate()),
      year: String(new Date(doc.dateAdded).getFullYear()),
      type: lang === `en` ? `book` : `spanish_translation`,
      title: lang === `en` ? doc.title : `${doc.title} — (Spanish)`,
      description:
        lang === `en`
          ? `Download free eBook or pdf, or purchase a paperback at cost.`
          : `${doc.authorName}'s ${doc.title} is now translated and available on the Spanish site.`,
      url: `/${doc.authorSlug}/${doc.slug}`,
      date: doc.dateAdded,
    }));
}
...
function dateFields(
  ...
  if (lang === `es`) {
    month = months[lang][months[`en`].indexOf(month)]?.substring(0, 3) || ``;
  }
